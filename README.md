...
完整端口转发使用原子事务管理 NAT、UFW 路由规则、状态登记和脚本拥有的 IPv4 forwarding 变更。一次确认的 TCP+UDP 批次要么全部通过本机验证，要么恢复事务前快照；无法验证回滚时会进入保护锁，禁止继续修改，但保留状态检查、诊断导出和恢复操作。NAT 与 UFW 均按字段语义验证；失败时会显示具体阶段、错误代码、预期与实际值，并保存回滚前现场和独立的回滚验证结果。
